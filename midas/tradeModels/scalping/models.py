...
class ScalpModel(object):
  ...
  def checkup(self):
    ...
    if (
      ...
      self._l.info(f'cancelling missed buy order {order.id} as {order.limit_price} (current price is {last_price})')
      self._cancel_order()
  ...
  def _calc_buy_signal(self) -> bool:
    ...
    if closes[-2] < moving_avg[-2] and closes[-1] > moving_avg[-1]:
      self._l.info(f'buy signal: closes[-2] {closes[-2]} < moving_avg {moving_avg[-2]} closes[-1] {closes[-1]} > moving_avg[-1] {moving_avg[-1]}')
      return True
    else:
      self._l.info(f'closes[-2:] = {closes[-2:]}, mavg[-2:] = {mavg[-2:]}')
      return False
  ...
  def on_bar(self, bar):
    ...
    self._l.info(f'received bar. start = {bar.start}, close = {bar.close}, len = {bar_len}')
    if bar_len < 21:
      return
